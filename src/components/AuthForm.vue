<template>
  <header class="bg-red-900 text-white py-2 px-4 shadow-md">
    ...
    >
      ...
      <form @submit.prevent="handleSubmit">
        ...
        <p class="text-center font-extrabold">
          {{ alternateText }}
          <router-link class="text-red-500 hover:underline" :to="alternateLink">{{
            alternateLinkText
          }}</router-link>
        </p>
      </form>
...
export default {
  ...
  methods: {
    ...
    async submitLogin() {
      ...
      if (valid) {
        try {
          const response = await axios.post('https://dummyjson.com/c/b9a5-5f1e-43f6-a497', {
    ...
    async submitSignup() {
      ...
      if (valid) {
        try {
          const response = await axios.post('https://dummyjson.com/c/610a-9f25-4de6-8ee0', {
            email: this.formData.email,
            password: this.formData.password
          })
          console.log(response.data)
